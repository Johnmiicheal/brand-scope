...
export async function createSearchRecord(
  ...
  try {
    ...
    console.log('Created search with ID:', searchId, 'for brand:', brandName || 'none', 'with engine:', searchData.engine);
    
    return searchId;
  } catch (error) {
    console.error('Failed to create search record:', error);
    throw new Error(`Failed to create search record: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function performGoogleSearch(
  ...
  console.log(`Performing Google search: query=${query}, engine=${engine}, location=${location || 'United States'}, includeAiOverview=${includeAiOverview}`);
    ...
    return data.ai_overview.text_blocks
      .map((block) => {
        if (block.type === "paragraph" && block.snippet) {
          ...
          return block.list.map(item => `${item.title}: ${item.snippet}`).join('\n');
        }
        return '';
      })
      .filter(Boolean)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { query, engine = 'google', includeAiOverview = true, brandName, monitoringId, location } = body;
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: `Failed to retrieve search history: ${error instanceof Error ? error.message : String(error)}` },
      { status: 500 }
    );
  }
}
