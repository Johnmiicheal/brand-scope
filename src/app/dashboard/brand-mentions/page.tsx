...
export default function CitationsPage() {
  ...
  const [analysisData, setAnalysisData] = useState<CitationAnalysisData | null>(
    null
  );
      ...
      const fetchAnalysis = async () => {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(
              errorData.error || "Failed to fetch citation analysis"
            );
          }
    ...
    <div className="mx-auto p-4 md:p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Review where your brand is mentioned during initial analysis and
          discover new ones based on AI-generated search prompts.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1 bg-background">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Quote className="h-5 w-5 text-blue-500" /> Initial Mentions
            </CardTitle>
            <CardDescription>
              Citations found during the last brand analysis run (
              {initialCitations.length} found).
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="bg-background">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <SearchCode className="h-5 w-5 text-purple-500" /> AI-Generated
                Search Prompts
              </CardTitle>
              <CardDescription>
                Potential search queries used to find new mentions.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoadingAnalysis ? (
                ...
                <p className="text-sm text-red-500">
                  Error loading prompts: {analysisError}
                </p>
          ...
          <Card className="bg-background">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <ListChecks className="h-5 w-5 text-teal-500" /> Discovered
                Brand Mentions
              </CardTitle>
              <CardDescription>
                Results from searching the web with the generated prompts (
                {analysisData?.exa_results?.length ?? 0} found).
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoadingAnalysis ? (
                ...
                <p className="text-sm text-red-500">
                  Error loading results: {analysisError}
                </p>
              ) : analysisData?.exa_results &&
                analysisData.exa_results.length > 0 ? (
                    ...
                    {analysisData.exa_results.map((result, index) => (
                      ...
                      >
                        <Link
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="font-medium text-primary hover:underline flex items-center gap-1 mb-1"
                        >
