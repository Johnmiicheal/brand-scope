...
const CustomNode = ({ nodeDatum, isMobile }: any) => (
  ...
  >
    <motion.div
      ...
      className="bg-muted p-2 md:p-4 rounded-xl border-2 
                 hover:shadow-md hover:border-blue-500/40 transition-all duration-300 
                 backdrop-blur-sm"
    >
      <div className="font-semibold text-[11px] md:text-sm text-white/80 mb-1 md:mb-1.5 line-clamp-1">
        {nodeDatum.name}
      </div>
      {nodeDatum.description && (
        <div className="text-[9px] md:text-xs text-white/50 line-clamp-2 leading-relaxed">
          {nodeDatum.description}
        </div>
      )}
    </motion.div>
...
const CompanyMindMap: React.FC<CompanyMindMapProps> = ({ data }) => {
  ...
  return (
    <div className="w-full h-[400px] md:h-[700px] bg-background rounded-md shadow-inner relative 
                    border">
      <Tree
        data={treeData}
        ...
        pathClassFunc={() => 'stroke-white stroke-[1.5px] transition-all duration-300'}
