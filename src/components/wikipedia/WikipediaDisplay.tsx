...
const WikipediaDisplay: React.FC<WikipediaDisplayProps> = ({ data, websiteUrl }) => {
  ...
  return (
    <div className="bg-background border rounded-lg shadow-md p-8 transition-all duration-300 hover:shadow-lg">
      {/* Header */}
      <div className="flex items-center gap-4 mb-8 border-b pb-4">
        ...
        <div>
          <h2 className="text-2xl font-semibold text-gray-200">Wikipedia</h2>
      ...
      {summary && (
        <div className="mb-8">
          <p className="text-white/80 leading-relaxed text-lg">{summary}</p>
        </div>
      )}
      ...
      {Object.keys(companyInfo).length > 0 && (
        <div className="mb-8">
          ...
          <div className="grid md:grid-cols-2 gap-6 bg-muted/20 p-6 rounded-lg">
            {Object.entries(companyInfo).map(([key, value]) => value && (
              <div key={key} className="flex flex-col">
                <span className="text-sm text-muted font-medium mb-1 capitalize">
          ...
          <div className="space-y-4">
            {timeline
              ...
              <div key={index} className="flex gap-4 items-start group">
                <div className="min-w-[80px] bg-blue-500/10 px-3 py-2 rounded-full text-blue-500 font-medium text-sm text-center transition-colors">
                  {event.year}
                </div>
                <p className="text-white/70 pt-1">{event.event}</p>
              </div>
            ))}
          </div>
          {timeline.length > 3 && (
            <button
              ...
              className="mt-6 flex items-center gap-2 text-blue-500 hover:text-blue-500/80 transition-colors"
            >
              {showAllEvents ? (
                <>
